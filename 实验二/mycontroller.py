...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        match_fields={
            "hdr.ipv4.dstAddr": (dst_ip_addr, 32) #如果包头对应的hdr.ipv4.dstAddr字段与参数中的dst_ip_addr匹配，就执行这一条表项的对应动作。后面的32是掩码，表示前32bit都是网络号。
        },
        action_name="MyIngress.myTunnel_ingress",
        action_params={        #设置匹配成功对应的动作名，该动作参数为传入的tunnel_id
            "dst_id": tunnel_id,
        })
    ...
    table_entry = p4info_helper.buildTableEntry(  #提供了实体的名字到ID数字转换的方法(buildTableEnrty函数)
...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            table_name = p4info_helper.get_tables_name(entry.table_id) #获取入口表ID
            print('%s: ' % table_name, end=' ')	#打印表名
            for i in entry.match: #对于每个匹配项
                print(p4info_helper.get_match_field_name(table_name, i.field_id), end=' ')
                print('%r' % (p4info_helper.get_match_field_value(i),), end=' ')
            action = entry.action.action #获取动作
            action_name = p4info_helper.get_actions_name(action.action_id) #获取动作名
            print('->', action_name, end=' ') #打印动作名
            for j in action.params: #对于动作中的每个参数
