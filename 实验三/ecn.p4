...
control MyEgress(inout headers hdr,
    ...
    apply {
         ...
         */
        if ( hdr.ipv4.ecn == 1 || hdr.ipv4.ecn == 2 ){  //ECN为1或者2
            if (standard_metadata.enq_qdepth >= ECN_THRESHOLD){ //与阈值进行比较
                hdr.ipv4.ecn = 3;  //设置其为3
            }
        }
    }
}
...
control MyComputeChecksum(inout headers hdr, inout metadata meta) {
    apply {
        ...
        update_checksum(
            ...
            { hdr.ipv4.version,
              hdr.ipv4.ihl,
              hdr.ipv4.diffserv, //tos拆分为 diffserve 与  ecn
              hdr.ipv4.ecn,
              hdr.ipv4.totalLen,
